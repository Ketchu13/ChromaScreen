...
class CoPrintExtrudersScreen(ScreenPanel, metaclass=Singleton):
    ...
    def __init__(self, screen, title):
        ...
        self.extruderImage = self._gtk.Image("ext_1", self._screen.width *.07, self._screen.width *.07)
        ...
        self.content_box.pack_start(extrusionSpeedButtonBox, False, True, 0)
        self.content_box.pack_start(selectedExtruderButtonBox, False, True, 0)
    ...
    def chanceExtruder(self, eventBox, gparam, extruder):
        ...
        oldIndex = next((i for i, item in enumerate(self.extruders) if
                         item['Extrude'] == self._printer.data["toolhead"]["extruder"]), -1)
        ...
        svg_file = f"styles/z-bolt/images/{self.extruders[index]['Icon']}.png"
        pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(svg_file, self._gtk.content_width * .15 , self._gtk.content_height * .15)
